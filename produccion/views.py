...
def _pendientes_del_dia(usuario):
    ...
    from core.servicios import almacen_terminado as servicio_terminado
    ...
    # Producto terminado bajo su mínimo. El cálculo existía desde que se
    # pusieron los mínimos, pero no salía en ninguna pantalla: había que
    # acordarse de entrar a mirarlo, y una alerta que hay que ir a buscar no es
    # una alerta. Aquí es donde el taller mira lo que hay que atender hoy.
    #
    # Es lo que evita el caso que contó el taller: piden cuarenta vigas, hay
    # treinta, y nadie se entera hasta la mañana siguiente, cuando ya hay que
    # fabricar diez con prisa.
    contar(
        "Producto bajo mínimo", "productos terminados por debajo de lo que hay que tener",
        "bi-graph-down-arrow", "catalogos:producto_terminado",
        lambda: len(servicio_terminado.alertas()),
        tono="aviso", cuando_cero="Ningún producto terminado bajo mínimo",
    )
