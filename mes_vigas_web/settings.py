"""
Django settings for mes_vigas_web project.
...
SECRET_KEY = 'django-insecure-&0%!i$*7b#^d!u^lz1!f+1pg$i=cmlm5m4=^bf5_4vz)8)zw7u'

# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = os.getenv("DJANGO_DEBUG", "1") == "1"

allowed_hosts = os.getenv("DJANGO_ALLOWED_HOSTS", "127.0.0.1,localhost,192.168.50.92").strip()
...
DATABASES = {
    ...
    'mes': {
        ...
        'USER': (
            'postgres'
            if (os.getenv('MES_DB_PASSWORD', '').strip() in {'', 'TU_PASSWORD_AQUI'})
            else os.getenv('MES_DB_USER', 'postgres').strip()
        ),
        'PASSWORD': (
            'elemetic'
            if (os.getenv('MES_DB_PASSWORD', '').strip() in {'', 'TU_PASSWORD_AQUI'})
            else os.getenv('MES_DB_PASSWORD', '').strip()
        ),
